...
import com.example.fundoonotes.service.NoteCheckListService;
...
public class NoteController {
    ...
    public NoteController(NoteService noteService,
                          NoteExcelBatchService noteExcelBatchService,
                          NoteCheckListService noteCheckListService) {
    ...
    // UC-12: Get checklist items for a note
    @GetMapping("/{noteId}/noteCheckLists")
    public ResponseEntity<List<CheckListResponse>> getCheckLists(@PathVariable int noteId,
                                                                 Authentication authentication) {
        return ResponseEntity.ok(noteCheckListService.getItems(noteId, authentication.getName()));
    }

    // UC-12: Add checklist item to a note
    @PostMapping("/{noteId}/noteCheckLists")
    public ResponseEntity<CheckListResponse> addCheckList(@PathVariable int noteId,
                                                          @Valid @RequestBody CheckListRequest request,
                                                          Authentication authentication) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(noteCheckListService.addItem(noteId, request, authentication.getName()));
    }

    // UC-12: Update checklist item
    @PutMapping("/{noteId}/noteCheckLists/{itemId}")
    public ResponseEntity<CheckListResponse> updateCheckList(@PathVariable int noteId,
                                                             @PathVariable int itemId,
                                                             @Valid @RequestBody CheckListRequest request,
                                                             Authentication authentication) {
        return ResponseEntity.ok(noteCheckListService.updateItem(
                noteId, itemId, request, authentication.getName()));
    }

    // UC-12: Soft delete checklist item
    @DeleteMapping("/{noteId}/noteCheckLists/{itemId}")
    public ResponseEntity<String> deleteCheckList(@PathVariable int noteId,
                                                  @PathVariable int itemId,
                                                  Authentication authentication) {
        return ResponseEntity.ok(noteCheckListService.deleteItem(noteId, itemId, authentication.getName()));
    }
}
