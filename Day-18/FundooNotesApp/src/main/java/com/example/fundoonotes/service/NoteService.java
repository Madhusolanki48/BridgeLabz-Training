...
import com.example.fundoonotes.messaging.ReminderMessage;
import com.example.fundoonotes.messaging.ReminderQueueProducer;
import com.example.fundoonotes.messaging.NoteShareEventProducer;
...
public class NoteService {
    ...
    public NoteService(NoteRepository noteRepository, UserRepository userRepository,
                       ...
                       ReminderQueueProducer reminderQueueProducer,
                       NoteShareEventProducer noteShareEventProducer) {
        this.noteRepository = noteRepository;
        this.userRepository = userRepository;
        this.noteLabelRepository = noteLabelRepository;
        this.reminderQueueProducer = reminderQueueProducer;
    ...
    // UC-10: Publish a note-shared event through RabbitMQ
    public String shareNote(int noteId, ShareNoteRequest request, String email) {
        Note note = getNote(noteId, email);
        noteShareEventProducer.sendNoteSharedEvent(
                note.getNoteId(),
                email,
                request.getCollaboratorEmail()
        );
        return "Note share event sent successfully";
    }

    // Permanently delete only after the note is already in trash
    ...
    // UC-8: Add or update reminders
    public NoteResponse addUpdateReminder(ReminderRequest request, String email) {
        Note note = getNote(request.getNoteId(), email);
        List<LocalDateTime> reminders = parseReminders(request.getReminder());

        note.getReminders().clear();
        note.getReminders().addAll(reminders);
        Note savedNote = noteRepository.save(note);

        reminders.forEach(reminder ->
                reminderQueueProducer.send(new ReminderMessage(savedNote.getNoteId(), email, reminder)));
        return mapToResponse(savedNote);
    }

    // UC-8: Remove reminders
    public NoteResponse removeReminder(ReminderRequest request, String email) {
        Note note = getNote(request.getNoteId(), email);
        List<LocalDateTime> reminders = parseReminders(request.getReminder());

        note.getReminders().removeAll(reminders);
        return mapToResponse(noteRepository.save(note));
    }

    // UC-8: Get notes having reminders
    ...
    private List<LocalDateTime> parseReminders(List<String> reminders) {
        return reminders.stream()
                .filter(reminder -> reminder != null && !reminder.isBlank())
    ...
    private NoteResponse mapToResponse(Note note) {
        ...
        List<CheckListResponse> checkLists = note.getCheckLists()
                .stream()
                .filter(item -> !item.isDeleted())
                .map(this::mapCheckListToResponse)
                .toList();
    ...
    private CheckListResponse mapCheckListToResponse(NoteCheckList item) {
